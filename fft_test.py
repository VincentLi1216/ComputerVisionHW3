...
# Correct the shifting issue by moving zero frequency to the center
fshift_centered = np.fft.ifftshift(fshift)  # This is the correct location for the shift

# Inverse FFT to get the filtered image back in spatial domain
img_freq = np.fft.ifft2(fshift_centered)
img_filtered = np.real(img_freq)
img_filtered = (img_filtered - img_filtered.min()) / (img_filtered.max() - img_filtered.min()) * 255  # Normalize the image
